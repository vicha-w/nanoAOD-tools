...
void single_muon_postprocessor(TString infilename, TString outfilename, bool isData=false) // outfilename must not have .root suffix! 
{
    ...
    for (int i_variant = 0; i_variant < (isData ? 1 : 7); i_variant++)
    {
        ...
        for (int ievent=0; ievent<intree->GetEntries(); ievent++)
        {
            ...
            for (int bjet=0; bjet<*(nbjets_pointers[i_variant]); bjet++)
            {
                ...
                Float_t deltaphi_bjet_muon = deltaPhi(infriends->tightRelIso_tightID_Muons_phi[0], bjets_phi_pointers[i_variant][bjet]);
